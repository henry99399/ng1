...
import com.cjsz.tech.enums.Language;
...
public class EnumLanguageHandler extends BaseTypeHandler<Language> {

    private Class<Language> type;
     ...
     */
    public EnumLanguageHandler(Class<Language> type) {
        if (type == null) {
            throw new IllegalArgumentException("Type argument cannot be null");
        }
        this.type = type;
        Language[] enums = type.getEnumConstants();
        if (enums == null) {
    ...
    public void setNonNullParameter(PreparedStatement ps, int i, Language parameter, JdbcType jdbcType) throws SQLException {
        ...
        ps.setString(i, parameter.getAbbr());
    }
     ...
     *
     * @param abbr 数据库中存储的自定义abbr属性
     * @return abbr对应的枚举类
     */
    private Language locateEnumStatus(String abbr) {
        Language language = Language.fromAbbr(abbr);
        if (language == null) {
            throw new IllegalArgumentException("未知的枚举类型：" + abbr + ",请核对" + type.getSimpleName());
        }
        return language;
    }
}
