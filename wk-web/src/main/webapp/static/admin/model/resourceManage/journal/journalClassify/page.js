...
myApp.controller('journalClassifyController', function ($rootScope, $scope, services, $sce, $stateParams) {
    $scope.services = services;
    //期刊分类树
    services["_journal_Classify_Tree"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/getTree', param, "POST");
    }
    //期刊分类新增/修改
    services["_journal_add"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/save_journal_cat', param, "POST");
    ...
    services["_journal_del"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/delete_journal_cat', param, "POST");
        //return $rootScope.serverAction(ctxPath + '/static/admin/json/data_cmsTemplate.json', param, "POST");
    }
    //期刊分类列表
    services["_journal_org_fenpei"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/json/orgList', param, "POST");
    }
    //期刊分类 - 添加
    services["_add_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/json/addOrg', param, "POST");
    }
    //期刊分类 - 移除
    services["_remove_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/json/removeOrg', param, "POST");
    }
    //期刊分类 - 启用/停用
    services["_enabled_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/json/updateEnabled', param, "POST");
    }
    //期刊分类 - 是否显示
    services["_show_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/journalCat/json/updateShow', param, "POST");
    }
    ...
    $scope.tableControl = {
        config: {
            ...
            columns: [
                {field: 'journal_cat_name', title: "名称", align: 'left'},
                {field: 'create_time', title: "创建时间"},
                {
                    field: 'is_show', title: "是否显示",
                    formatter: function (value, row, index) {
                        var bool = row.is_show == 1 ? 'layui-form-onswitch' : '';
                        return '<div class="layui-unselect layui-form-switch ' + bool + '" onclick="updateShow(' + row.journal_cat_id + ', event)"><i></i></div>';
                    }
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        return '<div class="layui-unselect layui-form-switch ' + bool + '" onclick="updateEnabled(' + row.journal_cat_id + ', event)"><i></i></div>';
                    }
                },
                {
                    field: 'org_count', title: "使用机构", formatter: function (value, row, index, data) {
                    return '<a href="javascript:void(0)" onclick="fenpei(' + row.journal_cat_id + ',event)">'+ value +'</a>';
                }
                },
                {
                    ...
                    if (index > 0 && data.length > 1) {
                        action += '<a href="javascript:void(0)" class="btn-icon" title="排序置顶" onclick="order_top(' + row.journal_cat_id + ',' + index + ',\'top\', event)"><i class="iconfont icon-dingbu"></i></a>';
                        action += '<a href="javascript:void(0)" class="btn-icon" title="排序上升" onclick="order_top(' + row.journal_cat_id + ',' + index + ',\'up\', event)"><i class="iconfont icon-shangsheng1"></i></a>';
                    }
                    if (index < data.length - 1) {
                        action += '<a href="javascript:void(0)" class="btn-icon" title="排序下降" onclick="order_top(' + row.journal_cat_id + ',' + index + ',\'down\', event)"><i class="iconfont icon-xiajiang"></i></a>';
                    }
    ...
    $("#resTree").tree({
        ...
        "onSelect": function (node) {
            $scope.selRow.journal_cat_pid = node.id;
    ...
    fenpei = function (cat_id, event) {
        ...
        $scope.layer_export = layer.open({
            ...
            title: "分类使用机构",
            area: ["700px", "600px"],
            content: $("#journalClassify_org")
        });
        $scope.fenpeiParam = {
            journal_cat_id: cat_id,
            project_id: null,
            searchtext: null,
            pageNum: 1,
            pageSize: 10,
    ...
    $scope._fenpeiData = function (num) {
        ...
        services._journal_org_fenpei($scope.fenpeiParam).success(function (res) {
    ...
    $scope.add_cat_org = function (row) {
        row.journal_cat_id = $scope.fenpeiParam.journal_cat_id;
    ...
    //改变排序
    order_top = function (journal_cat_id, index, type, event) {
        $rootScope.stopEvent(event);
        angular.forEach($scope.tableControl.data, function (item, index) {
            if (item.journal_cat_id == journal_cat_id) {
    ...
    //加载
    $scope.load = function () {
        services._journal_Classify_Tree().success(function (res) {
            var arrdata = [];
    ...
    $scope.row_update = function () {
        ...
        services._journal_Classify_Tree().success(function (res) {
            ...
            //隐藏当前节点
            var nowNN = $("#resTree").tree("find", $scope.selRow.journal_cat_id);
            $("#resTree").tree("remove", nowNN.target);
        })
    }
    //提交
    $scope._form_submit = function (bool) {
        if (!$scope.selRow.journal_cat_name) {
            layer.alert("请填写分类名称")
            return false;
        }

        if ($scope.selRow.journal_cat_pid == $scope.selRow.journal_cat_id) {
        ...
        services._journal_add($scope.selRow).success(function (res) {
            if (res.code == 0) {
                if (bool) {
                    ...
                    $scope.selRow = {
                        journal_cat_pid: $scope.selRow.journal_cat_pid,
    ...
    $scope.delRow = function () {
        var journal_cat_ids = new Array();
        var mark = null;
        angular.forEach($scope.tableControl.rows, function (item, index) {
            if (item.select) {
                journal_cat_ids.push($scope.tableControl.data[item.index].journal_cat_id)
            }
        });
        if (journal_cat_ids.length == 0) {
            ...
            layer.confirm('删除后数据无法找回,确认删除吗？', {
                ...
                services._journal_del({journal_cat_ids: journal_cat_ids, mark: mark}).success(function (res) {
                    if (res.code == 0) {
                        layer.msg("删除成功")
                        $scope.load();
                    }
                        ...
                        layer.confirm(res.message, {
                            ...
                            mark = 'del'
                            services._journal_del({
                                journal_cat_ids: journal_cat_ids,
                                mark: mark
