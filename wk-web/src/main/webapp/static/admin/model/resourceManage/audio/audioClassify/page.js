...
myApp.controller('audioClassifyController', function ($rootScope, $scope, services, $sce, $stateParams) {
    $scope.services = services;
    //音频分类树
    services["_audio_Classify_Tree"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/getTree', param, "POST");
    }
    //音频分类新增/修改
    services["_audio_add"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/save_audio_cat', param, "POST");
    ...
    services["_audio_del"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/delete_audio_cat', param, "POST");
        //return $rootScope.serverAction(ctxPath + '/static/admin/json/data_cmsTemplate.json', param, "POST");
    }
    //音频分类列表
    services["_audio_org_fenpei"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/json/orgList', param, "POST");
    }
    //音频分类 - 添加
    services["_add_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/json/addOrg', param, "POST");
    }
    //音频分类 - 移除
    services["_remove_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/json/removeOrg', param, "POST");
    }
    //音频分类 - 启用/停用
    services["_enabled_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/json/updateEnabled', param, "POST");
    }
    //音频分类 - 是否显示
    services["_show_org_cat"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/audioCat/json/updateShow', param, "POST");
    }
    ...
    $scope.tableControl = {
        config: {
            ...
            columns: [
                {field: 'audio_cat_name', title: "名称", align: 'left'},
                {field: 'create_time', title: "创建时间"},
                {
                    field: 'is_show', title: "是否显示",
                    formatter: function (value, row, index) {
                        var bool = row.is_show == 1 ? 'layui-form-onswitch' : '';
                        return '<div class="layui-unselect layui-form-switch ' + bool + '" onclick="updateShow(' + row.audio_cat_id + ', event)"><i></i></div>';
                    }
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        return '<div class="layui-unselect layui-form-switch ' + bool + '" onclick="updateEnabled(' + row.audio_cat_id + ', event)"><i></i></div>';
                    }
                },
                {
                    field: 'org_count', title: "使用机构", formatter: function (value, row, index, data) {
                    return '<a href="javascript:void(0)" onclick="fenpei(' + row.audio_cat_id + ',event)">' + value + '</a>';
                }
                },
                {
                    ...
                    if (index > 0 && data.length > 1) {
                        action += '<a href="javascript:void(0)" class="btn-icon" title="排序置顶" onclick="order_top(' + row.audio_cat_id + ',' + index + ',\'top\', event)"><i class="iconfont icon-dingbu"></i></a>';
                        action += '<a href="javascript:void(0)" class="btn-icon" title="排序上升" onclick="order_top(' + row.audio_cat_id + ',' + index + ',\'up\', event)"><i class="iconfont icon-shangsheng1"></i></a>';
                    }
                    if (index < data.length - 1) {
                        action += '<a href="javascript:void(0)" class="btn-icon" title="排序下降" onclick="order_top(' + row.audio_cat_id + ',' + index + ',\'down\', event)"><i class="iconfont icon-xiajiang"></i></a>';
                    }
    ...
    $scope.add_cat_org = function (row) {
        row.audio_cat_id = $scope.fenpeiParam.audio_cat_id;
     ...
     * @param type
     */
    order_top = function (audio_cat_id, index, type, event) {
        $rootScope.stopEvent(event);
        angular.forEach($scope.tableControl.data, function (item, index) {
            if (item.audio_cat_id == audio_cat_id) {
    ...
    //加载
    $scope.load = function () {
        services._audio_Classify_Tree().success(function (res) {
            var arrdata = [];
    ...
    $scope.row_update = function () {
        ...
        services._audio_Classify_Tree().success(function (res) {
            ...
            $("#resTree").tree("loadData", allData);
            //隐藏当前节点
            var nowNN = $("#resTree").tree("find", $scope.selRow.audio_cat_id);
            $("#resTree").tree("remove", nowNN.target);
        })
    }
    //提交
    $scope._form_submit = function (bool) {
        if (!$scope.selRow.audio_cat_name) {
            layer.alert("请填写分类名称")
            return false;
        }
    ...
    $scope.delRow = function () {
        var audio_cat_ids = new Array();
        var mark = null;
        angular.forEach($scope.tableControl.rows, function (item, index) {
            if (item.select) {
                audio_cat_ids.push($scope.tableControl.data[item.index].audio_cat_id)
            }
        });
        if (audio_cat_ids.length == 0) {
            ...
            layer.confirm('删除后数据无法找回,确认删除吗？', {
                ...
                services._audio_del({audio_cat_ids: audio_cat_ids, mark: mark}).success(function (res) {
                    if (res.code == 0) {
                        layer.msg("删除成功")
                        $scope.load();
                    }
