...
function loadPage(page, pageElement) {
    ...
    img.attr('src', ctxPath + imgList[page - 1].image_small_url);//image_small_url
    img.load(function () {
        if (!img.attr('maxEnd')) {
            img.attr('maxEnd', true);
            console.log('开始加载大图')
            createElelentImg(this, page);
            //img.attr('src', ctxPath + imgList[page - 1].image_url);
        }
    })
    console.log("加载页面-小图")
...
var imgElementArray = [];
function createElelentImg(img, page) {
    imgElementArray.push({
        img: img,
        page: page
    })
    var dl_img = document.createElement("img");
    dl_img.style.display = "none";
    dl_img.src = ctxPath + imgList[page - 1].image_url;
    dl_img.setAttribute('index',imgElementArray.length - 1);
    dl_img.onload = function () {
        var obj_index = parseInt(this.getAttribute('index'));
        imgElementArray[obj_index].img.src = this.src;
        console.log('高清加载完毕:' + this.src)
    }
    document.body.appendChild(dl_img);
}
// Zoom in / Zoom out
...
function loadSmallPage(page, pageElement) {
    ...
    // Loadnew page
    // img.attr('src', 'pages/' + page + '.png');
    //img.attr('src', ctxPath + imgList[page - 1].image_url);//image_small_url
    console.log("缩小")
}
