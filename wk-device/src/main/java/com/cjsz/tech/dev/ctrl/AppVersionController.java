...
public class AppVersionController {
    ...
    @Autowired
    DeviceVersionRelService deviceVersionRelService;
    ...
    public Object appVersionPageList(HttpServletRequest request, @RequestBody PageConditionBean pageConditionBean){
        try{
            ...
            Object data = appVersionService.pageQuery(sort, pageConditionBean);
    ...
    public Object appVersionList(HttpServletRequest request){
        ...
        try{
            List<AppVersion> appVersions = appVersionService.getAllVersions();
            JsonResult result = JsonResult.getSuccess(Constants.LOAD_SUCCESS);
    ...
    @ResponseBody
    public Object update_appNav(HttpServletRequest request, @RequestBody AppVersion appVersion){
        try{
            ...
            if (appVersion.getVersion_id() != null) {
                AppVersion version = appVersionService.selectByVersionIds(appVersion.getVersion_id().toString()).get(0);
    ...
    @ResponseBody
    public Object update_enabled(HttpServletRequest request, @RequestBody AppVersion appVersion){
        try{
            ...
            appVersion.setUpdate_time(new Date());
            appVersionService.updateEnabled(appVersion);
            result.setData(appVersion);
            return result;
        }catch (Exception e){
            e.printStackTrace();
    ...
    public Object delete_appVersions(HttpServletRequest request, @RequestBody DelVersionBean bean){
        try{
            ...
            List<AppVersion> appVersions = appVersionService.selectByVersionIds(appVersion_ids_str);
            for(AppVersion appVersion : appVersions){
                if(appVersion.getEnabled() == 1){
                    return JsonResult.getError("版本正在使用，不能删除");
                }
            }
            appVersionService.deleteByVersionIds(appVersion_ids_str);
    ...
    public Object selectDevices(HttpServletRequest request, @RequestBody DevicePageBean pageConditionBean) {
        ...
        try{
            ...
            Object data = appVersionService.selectDevices(sort, pageConditionBean);
            JsonResult result = JsonResult.getSuccess(Constants.LOAD_SUCCESS);
            result.setData(data);
            return result;
        }catch (Exception e){
